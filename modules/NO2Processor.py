...
def process(
    header_length, initial, exceedance, background_name, input_data, output_filename
):
    ...
    :type exceedance: int.
    :param background_name: This should be a string representing input background NO2 filename.
    ...
    try:
        background = pd.read_csv(background_name)
        temp_list = []
        data = pd.DataFrame
        ...
        data["background"] = list(
            map(lambda x: float(x * 0.9583333), background["Ozone"])
        )
        data = data.fillna(0)
        def olmfunction(row):
            back = float(row[-1])
        ...
        outdf = pd.DataFrame(
            {
                "Max of Sensor": data.max(),
                "Average of Sensor": data.mean(),
        ...
        backquestion = QMessageBox.question(
            None,
            "Background NO2 Statistics?",
            "Would you like to process Background NO2 statistics?",
