...
def Statistics_Generator(settings, header_length, input_data, output_filename):
    ...
    try:
        ...
        if settings["rolling"] and settings["max_rolling"]:
            ...
            temp_df = pd.DataFrame(
                {
                    col_name: data.rolling(window=settings["rolling_value"])
                    .mean()
                    .max()
                    .max()
                },
        ...
        ErrorBox(
            "Invalid Dataset",
            "Invalid dataset, please check input datasets\n" + str(err),
        )
