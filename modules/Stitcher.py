...
def Stitcher(filepath_list, filename, scale, headers_list, output_filename):
    ...
    """
    x = 1
    for folder, filename, scale, header in zip(
        filepath_list, filename, scale, headers_list
    ):
        ...
        try:
            ...
            if x == 1:
                ...
                if dataout.shape != data.shape:
                    ErrorBox(
                        "Datasets Size Error",
                        "Please review data set ("
                        + filename
                        + ") size (eg missing hour or day)",
                        "Expecting data set size of "
                        + str(dataout.shape)
                        + " \n Received: "
                        + str(data.shape),
                    )
                    ...
                    dataout.update(
                        dataout.iloc[:, headlen:].add(data.iloc[:, headlen:])
                    )
            del data
            ...
            ErrorBox(
                "File Not Found",
                "File not found, please specify file \nFile not found: " + filename,
                err,
            )
    try:
        ...
        ErrorBox(
            "Output Data File",
            "Invalid output dataset filename, please specify output dataset name",
            err,
        )
        ...
        ErrorBox(
            "Output Data Empty",
            "Output dataset empty, please check input datasets",
            err,
        )
